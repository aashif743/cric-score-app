...
const TournamentFormatListScreen = ({ navigation, route }) => {
  const { user } = useContext(AuthContext);
  const format = route.params?.format || 'quick';
  ...
  // Treat legacy tournaments without an explicit format as "quick"
  const filteredTournaments = tournaments.filter(
    (t) => (t.format || 'quick') === format
  );
  ...
  const handleCreate = () => {
    navigation.navigate('TournamentCreate', { format });
  };

  const handleTournamentPress = (t) => {
    const fmt = t.format || 'quick';
    if (fmt === 'knockout') {
      navigation.navigate('KnockoutSchedule', { tournamentId: t._id });
    } else if (fmt === 'league') {
      navigation.navigate('LeagueSchedule', { tournamentId: t._id });
    } else {
      navigation.navigate('TournamentDetail', { tournamentId: t._id });
    }
  };
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyState}>
      ...
      <TouchableOpacity style={styles.emptyButton} onPress={handleCreate} activeOpacity={0.8}>
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <GradientHeader
        title={FORMAT_TITLES[format] || 'Tournaments'}
        subtitle={`${filteredTournaments.length} ${filteredTournaments.length === 1 ? 'tournament' : 'tournaments'}`}
      ...
      {loading ? (
        ...
        <FlatList
          ...
          contentContainerStyle={[
            styles.listContent,
            filteredTournaments.length === 0 && styles.emptyListContent,
          ]}
...
const styles = StyleSheet.create({
  ...
  emptyTrophyContainer: { alignItems: 'center' },
  emptyTrophyCup: {
    width: 40, height: 30,
    borderWidth: 4, borderBottomWidth: 0, borderColor: '#fbbf24',
    borderTopLeftRadius: 6, borderTopRightRadius: 6,
    borderBottomLeftRadius: 16, borderBottomRightRadius: 16,
  },
  emptyTrophyBase: {
    width: 24, height: 6, backgroundColor: '#fbbf24', borderRadius: 3, marginTop: -2,
  },
  ...
  fabIcon: { width: 24, height: 24, justifyContent: 'center', alignItems: 'center' },
  fabHLine: {
    position: 'absolute', width: 20, height: 3,
    backgroundColor: '#fff', borderRadius: 1.5,
  },
  fabVLine: {
    position: 'absolute', width: 3, height: 20,
    backgroundColor: '#fff', borderRadius: 1.5,
  },
  ...
  modalTrashTop: {
    width: 28, height: 4, backgroundColor: '#ef4444', borderRadius: 2, marginBottom: 3,
  },
  modalTrashBody: {
    width: 22, height: 22,
    borderWidth: 3, borderColor: '#ef4444', borderTopWidth: 0,
    borderBottomLeftRadius: 4, borderBottomRightRadius: 4,
  },
  modalTitle: { fontSize: 22, fontWeight: '700', color: '#0f172a', marginBottom: 10 },
  modalMessage: {
...
export default TournamentFormatListScreen;
