...
const formatBowlers = (bowlersList = [], ballsPerOver) =>
  bowlersList.map((b) => {
    ...
    return {
      ...
      economyRate:
        totals.balls > 0
          ? ((totals.runs / totals.balls) * ballsPerOver).toFixed(2)
          : "0.00",
      ...totals,
    };
  });
...
const calculateNetRunRates = (innings1, innings2, settings) => {
  ...
  const runRate = (runs, overs) =>
    overs > 0 ? (runs / overs).toFixed(3) : "0.00";
  const team1Overs =
    innings1.wickets === settings.playersPerTeam - 1
      ? settings.overs
      : parseOvers(innings1.overs);
  const team2Overs =
    innings2.wickets === settings.playersPerTeam - 1
      ? settings.overs
      : parseOvers(innings2.overs);

  return {
    [innings1.teamName]: (
      runRate(innings1.runs, team1Overs) - runRate(innings2.runs, team2Overs)
    ).toString(),
    [innings2.teamName]: (
      runRate(innings2.runs, team2Overs) - runRate(innings1.runs, team1Overs)
    ).toString(),
  };
};
...
const determinePlayerOfMatch = (innings1, innings2) => {
  ...
  const topScorer = allBatters.reduce(
    (a, b) => (b.runs > (a?.runs || -1) ? b : a),
    null
  );
  const topWicketTaker = allBowlers.reduce(
    (a, b) => (b.wickets > (a?.wickets || -1) ? b : a),
    null
  );
...
const createInitialState = (matchSettings) => {
  ...
  const createTeam = (name, count) => ({
    name,
    batsmen: Array.from({ length: count }, (_, i) => ({
      id: uuidv4(),
      name: `Player ${i + 1}`,
    })),
    bowlers: Array.from({ length: count }, (_, i) => ({
      id: uuidv4(),
      name: `Player ${i + 1}`,
    })),
  });
...
const ScorecardPage = ({
  ...
  const handleUndo = () => {
    ...
    setTeams((currentTeams) => {
      ...
      const battingKey =
        updated.teamA.name === prevState.match.battingTeam ? "teamA" : "teamB";
      const bowlingKey = battingKey === "teamA" ? "teamB" : "teamA";

      const striker = updated[battingKey].batsmen.find(
        (b) => b.id === prevState.players.striker.id
      );
      const nonStriker = updated[battingKey].batsmen.find(
        (b) => b.id === prevState.players.nonStriker.id
      );
      const bowler = updated[bowlingKey].bowlers.find(
        (b) => b.id === prevState.players.bowler.id
      );
  ...
  const updateBowlerStats = (runs = 0, isWicket = false, countBall = true) => {
    ...
    setTeams((prevTeams) => {
      ...
      updated[currentBowlingTeamKey].bowlers = updated[
  ...
  const handleRuns = (runsScored) => {
    ...
    setTeams((prevTeams) => {
      ...
      const batsman = updated[battingKey].batsmen.find(
        (b) => b.id === strikerId
      );
      if (batsman) {
  ...
  const handleWicket = useCallback(
    (dismissalInfo) => {
      ...
      let dismissed =
        batsmanOut === "nonStriker" ? players.nonStriker : players.striker;
      let partner =
        batsmanOut === "nonStriker" ? players.striker : players.nonStriker;
  ...
  const onOverComplete = useCallback(() => {
    ...
    const completedOver = {
      ...
      maiden:
        players.bowler.currentSpell.runs === 0 &&
        players.bowler.currentSpell.balls === settings.ballsPerOver,
    ...
    localStorage.setItem(
      ...
      JSON.stringify([completedOver, ...allOversHistory])
    );
  ...
  useEffect(() => {
    ...
    const isOverComplete =
      match.balls > 0 && match.balls % settings.ballsPerOver === 0;
  ...
  const startSecondInnings = () => {
    ...
    const firstInningsData = {
      ...
      batting: formatBatsmen(
        teams[teams.teamA.name === match.battingTeam ? "teamA" : "teamB"]
          .batsmen
      ),
      bowling: formatBowlers(
        teams[teams.teamA.name === match.bowlingTeam ? "teamA" : "teamB"]
          .bowlers,
    ...
    const battingTeamKey =
      teams.teamA.name === newBattingTeam ? "teamA" : "teamB";
  ...
  const endMatch = () => {
    ...
    if (match.innings === 2) {
      if (match.runs > match.target - 1) {
        result = `${match.battingTeam} won by ${10 - match.wickets} wicket${
          10 - match.wickets > 1 ? "s" : ""
        }`;
      } else if (match.runs < match.target - 1) {
        const runDiff = match.target - match.runs - 1;
        result = `${match.bowlingTeam} won by ${runDiff} run${
          runDiff > 1 ? "s" : ""
        }`;
      } else {
        result = "Match Tied";
    ...
    const netRR = calculateNetRunRates(
      firstInnings,
      secondInnings || {},
      settings
    );
    ...
    <div className="scorecard-page">
      ...
      <div className="match-summary">
        ...
        <div className="batsmen-info">
          <div className="batsman striker">
            ...
            <p>
              {players.striker.runs} ({players.striker.balls})
            </p>
          </div>
          <div className="batsman non-striker">
            <span>{players.nonStriker.name}</span>
            <p>
              {players.nonStriker.runs} ({players.nonStriker.balls})
            </p>
          </div>
        </div>
        ...
        <div className="bowler-info">
          ...
          <p>
            {players.bowler.currentSpell.runs}-
            {players.bowler.currentSpell.wickets} (
            {formatOvers(
              players.bowler.currentSpell.balls,
              settings.ballsPerOver
            )}
            )
          </p>
      ...
      <div className="input-section">
        ...
        <div className="control-buttons">
          ...
          <button onClick={() => setSelectedAction("ChangeStriker")}>
            Change Striker
          </button>
          <button onClick={() => setShowPrevOversModal(true)}>
            View Over History
          </button>
        </div>
        ...
        <div className="score-buttons">
          {[0, 1, 2, 3, 4, 5, 6].map((run) => (
            <button key={run} onClick={() => handleRuns(run)}>
              {run}
            </button>
          ))}
        ...
        <div className="confirm-button">
          <button
            onClick={() => setShowModal(selectedAction)}
      ...
      <AnimatePresence>
        ...
        {showModal === "ManualRuns" && (
          <ManualRunModal
            onConfirm={handleRuns}
            onClose={() => setShowModal(null)}
          />
        )}
        {showPrevOversModal && (
          <OverHistoryModal
            data={allOversHistory}
            onClose={() => setShowPrevOversModal(false)}
          />
        )}
      </AnimatePresence>
    </div>
