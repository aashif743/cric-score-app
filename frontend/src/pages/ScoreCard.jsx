...
import InningsInfo from '../components/InningsInfo';
import BatsmanInfo from '../components/BatsmanInfo';
import BowlerInfo, { OverHistoryModal } from '../components/BowlerInfo';
import SummaryModal from '../components/SummaryModal'; // 1. Import the new mo
...
const createInitialState = (matchSettings) => {
  ...
  const teamAName =
    matchSettings?.teamA?.name || matchSettings?.teamA || "Team A";
  const teamBName =
    matchSettings?.teamB?.name || matchSettings?.teamB || "Team B";
  ...
  const initialStriker = initialTeams.teamA.batsmen[0] || {
    id: uuidv4(),
    name: "Player 1",
  };
  const initialNonStriker = initialTeams.teamA.batsmen[1] || {
    id: uuidv4(),
    name: "Player 2",
  };
  const initialBowler = initialTeams.teamB.bowlers[0] || {
    id: uuidv4(),
    name: "Player 1",
  };
...
const determinePlayerOfMatch = (innings1, innings2) => {
  ...
  const allPlayersBatting = [
    ...(innings1?.batting || []),
    ...(innings2?.batting || []),
  ];
  const topScorer = allPlayersBatting.reduce(
    (prev, curr) => ((curr.runs || 0) > (prev?.runs || -1) ? curr : prev),
    { runs: -1 }
  );
  ...
  const topWicketTaker = allBowlers.reduce(
    (prev, curr) => ((curr.wickets || 0) > (prev?.wickets || -1) ? curr : prev),
    { wickets: -1 }
  );
  ...
  // Fallback logic
  if (topWicketTaker && topWicketTaker.wickets > (topScorer?.runs || 0) / 15) {
    // Simple heuristic
...
const formatBatsmenData = (
  ...
  return batsmenListFromTeamsState.map((batsmanInTeam) => {
    ...
    if (
      currentStrikerFromPlayersState &&
      batsmanInTeam.id === currentStrikerFromPlayersState.id
    ) {
      processedBatsman = {
        ...batsmanInTeam,
      ...
      currentNonStrikerFromPlayersState &&
      batsmanInTeam.id === currentNonStrikerFromPlayersState.id
    ) {
      processedBatsman = {
        ...batsmanInTeam,
    ...
    const isEffectivelyOut =
      processedBatsman.isOut ||
    ...
    return {
      ...
      strikeRate:
        actualBalls > 0
          ? ((actualRuns / actualBalls) * 100).toFixed(2)
          : "0.00",
      status: isEffectivelyOut
        ? processedBatsman.outType || "Out"
        : atCreaseStatus
        ? "Not Out"
        : actualBalls > 0 || actualRuns > 0
        ? "Not Out"
        : "Did Not Bat",
...
const formatBowlersForSummary = (bowlersList = [], ballsPerOverSettings) => {
  return bowlersList.map((b) => {
    const allSpells = [
      ...(b.previousSpells || []),
      ...(b.currentSpell ? [b.currentSpell] : []),
    ];
    const totals = allSpells.reduce(
      (acc, spell) => ({
    ...
    return {
      id: b.id,
      name: b.name,
      overs: `${Math.floor(totals.balls / ballsPerOverSettings)}.${
        totals.balls % ballsPerOverSettings
      }`,
      runs: totals.runs,
      ...
      economyRate:
        totals.balls > 0
          ? ((totals.runs / totals.balls) * ballsPerOverSettings).toFixed(2)
          : "0.00",
    };
  });
};
...
const ScorecardPage = ({
  ...
  const overs = `${Math.floor(balls / settings.ballsPerOver)}.${
    balls % settings.ballsPerOver
  }`;
  const crr =
    balls > 0 ? (runs / (balls / settings.ballsPerOver)).toFixed(2) : "0.00";
  const remainingBalls = settings.overs * settings.ballsPerOver - balls;
  const remainingRuns = match.target - runs;
  const requiredRunRate =
    remainingBalls > 0
      ? (remainingRuns / (remainingBalls / settings.ballsPerOver)).toFixed(2)
      : "0.00";
  ...
  }, [match, players, overHistory, teams]);
  ...
  const rehydrateTeamsFromHistory = (settings, allOversHistory) => {
    ...
    const getPlayer = (teamName, playerName, playerType) => {
      const teamKey =
        rehydratedTeams.teamA.name === teamName ? "teamA" : "teamB";
      const playerList =
        playerType === "batsman"
          ? rehydratedTeams[teamKey].batsmen
          : rehydratedTeams[teamKey].bowlers;
  ...
  const sendMatchUpdate = useCallback(async () => {
    ...
    if (!user || !user.token) {
      console.error(
        "❌ Cannot update match - User is not logged in or token is missing."
      );
      return;
    }
    ...
    try {
      ...
      const currentBattingTeamKey =
        teams.teamA.name === match.battingTeam ? "teamA" : "teamB";
      const currentBowlingTeamKey =
        currentBattingTeamKey === "teamA" ? "teamB" : "teamA";

      if (!teams[currentBattingTeamKey] || !teams[currentBowlingTeamKey]) {
        console.error(
          "❌ Critical error: Could not determine current teams for update."
        );
        return;
      }
      ...
      const updatePayload = {
        ...
        inningsUpdate: {
          ...
          bowling: formatBowlersForUpdate(
            teams[currentBowlingTeamKey].bowlers,
      ...
      console.log(
        "✅ Match updated successfully on server:",
  ...
  const handleUndo = () => {
    if (history.length === 0) {
      console.warn("Undo clicked, but no history available.");
      return;
    }

    // Get the most recent state from the history array
    const previousState = history[history.length - 1];

    // ✅ FIX: Restore ALL relevant states directly from the snapshot
    setMatch(previousState.match);
    setPlayers(previousState.players);
    setTeams(previousState.teams); // This correctly reverts bowler spells and all player stats
    setOverHistory(previousState.overHistory);

    // Restore the ref for over-completion tracking
    lastCompletedOverBallCount.current =
      previousState.lastCompletedOverBallCount;

    // Remove the state we just restored from the history array
    setHistory((prev) => prev.slice(0, -1));

    // Reset any open modals or selected actions in the UI
    setShowModal(null);
        ...
        bowlers: updatedTeams[currentBowlingTeamKey].bowlers.map((bowler) => {
          if (bowler.id === bowlerId) {
            const updatedSpell = {
              ...
              wickets: isWicket
                ? bowler.currentSpell.wickets + 1
                : bowler.currentSpell.wickets,
            };
            ...
            if (countBall) {
              ...
              if (
                updatedSpell.balls % settings.ballsPerOver === 0 &&
                updatedSpell.runs === 0
              ) {
        ...
        return {
          ...
          bowler: {
            ...
            currentSpell: {
              ...
              wickets: isWicket
                ? prev.bowler.currentSpell.wickets + 1
                : prev.bowler.currentSpell.wickets,
              balls: countBall
                ? prev.bowler.currentSpell.balls + 1
                : prev.bowler.currentSpell.balls,
            },
          },
        };
      }
  ...
  const handleRuns = (runsScored) => {
    ...
    setTeams((prevTeams) => {
      ...
      const batsmanToUpdate = updatedTeams[currentBattingTeamKey]?.batsmen.find(
        (b) => b.id === strikerId
      );
      if (batsmanToUpdate) {
        ...
        if (runsScored === 4)
          batsmanToUpdate.fours = (batsmanToUpdate.fours || 0) + 1;
        if (runsScored === 6)
          batsmanToUpdate.sixes = (batsmanToUpdate.sixes || 0) + 1;
    ...
    setPlayers((prev) => {
      const updatedStriker = {
        ...
        fours:
          runsScored === 4
            ? (prev.striker.fours || 0) + 1
            : prev.striker.fours || 0,
        sixes:
          runsScored === 6
            ? (prev.striker.sixes || 0) + 1
            : prev.striker.sixes || 0,
      };
  ...
  const handleExtras = (type, runsFromExtras = 0) => {
    ...
    setMatch((prev) => {
      ...
      if (type === "NB")
        newExtras.noBalls = (newExtras.noBalls || 0) + teamRuns; // Only penalty is a "no ball" extra
      if (type === "BYE") newExtras.byes = (newExtras.byes || 0) + teamRuns;
      if (type === "LB")
        newExtras.legByes = (newExtras.legByes || 0) + teamRuns;
    ...
    setPlayers((prev) => {
      ...
      const runsPhysicallyRun =
        type === "WD" || type === "BYE" || type === "LB" || type === "NB"
          ? runsFromExtras
          : 0;
    ...
    setOverHistory((prev) => [
      ...prev,
      `${runsFromExtras > 0 ? runsFromExtras : ""}${type}`,
    ]);
    setShowModal(null);
    setSelectedAction(null);
  ...
  const handleTeamNameUpdate = (teamType, newName) => {
    const battingTeamKey = teams.teamA.name === match.battingTeam ? 'teamA' : 'teamB';
    const bowlingTeamKey = battingTeamKey === 'teamA' ? 'teamB' : 'teamA';

    const keyToUpdate = teamType === 'batting' ? battingTeamKey : bowlingTeamKey;

    // Update the master teams state
    setTeams(prevTeams => ({
        ...prevTeams,
        [keyToUpdate]: {
            ...prevTeams[keyToUpdate],
            name: newName
        }
    }));

    // Also update the match state which holds the current names
    setMatch(prevMatch => ({
        ...prevMatch,
        [teamType === 'batting' ? 'battingTeam' : 'bowlingTeam']: newName
    }));
};

const handleBatsmanNameUpdate = (playerId, newName) => {
    const currentBattingTeamKey = teams.teamA.name === match.battingTeam ? 'teamA' : 'teamB';

    // Update the master list in the `teams` state
    setTeams(prevTeams => {
        const newTeams = JSON.parse(JSON.stringify(prevTeams));
        const batsmenList = newTeams[currentBattingTeamKey].batsmen;
        const playerIndex = batsmenList.findIndex(p => p.id === playerId);
        if (playerIndex !== -1) {
            batsmenList[playerIndex].name = newName;
        }
        return newTeams;
    });

    // Update the active players in the `players` state
    setPlayers(prevPlayers => {
        const newPlayers = { ...prevPlayers };
        if (newPlayers.striker.id === playerId) {
            newPlayers.striker.name = newName;
        }
        if (newPlayers.nonStriker.id === playerId) {
            newPlayers.nonStriker.name = newName;
        }
        return newPlayers;
    });
};

const handleBowlerNameUpdate = (playerId, newName) => {
    const currentBowlingTeamKey = teams.teamA.name === match.bowlingTeam ? 'teamA' : 'teamB';

    setTeams(prevTeams => {
        const newTeams = JSON.parse(JSON.stringify(prevTeams));
        const bowlersList = newTeams[currentBowlingTeamKey].bowlers;
        const playerIndex = bowlersList.findIndex(p => p.id === playerId);
        if (playerIndex !== -1) {
            bowlersList[playerIndex].name = newName;
        }
        return newTeams;
    });
    ...
    setPlayers(prevPlayers => {
        const newPlayers = { ...prevPlayers };
        if (newPlayers.bowler.id === playerId) {
            newPlayers.bowler.name = newName;
        }
        return newPlayers;
    });
};
  ...
  const onOverComplete = useCallback(() => {
    ...
    const bowlerId = players.bowler.id;
    const currentBowlingTeamKey =
      teams.teamA.name === match.bowlingTeam ? "teamA" : "teamB";
    const bowlerInState = teams[currentBowlingTeamKey]?.bowlers.find(
      (b) => b.id === bowlerId
    );
    ...
    const completedOverData = {
      ...
      maiden:
        completedSpell.runs === 0 &&
        completedSpell.balls === settings.ballsPerOver,
    };

    setAllOversHistory((prev) => {
      const newHistory = [completedOverData, ...prev].slice(0, 20);
      localStorage.setItem(
        `oversHistory_${matchId}`,
  ...
  const handleWicket = useCallback(
    (dismissalInfo) => {
      ...
      const runsForBowler = isNoBall
        ? runsCompleted + settings.noBallRuns
        : isWide
        ? runsCompleted + settings.wideBallRuns
        : runsCompleted;
      ...
      setMatch((prev) => {
        const penaltyRuns = isNoBall
          ? settings.noBallRuns
          : isWide
          ? settings.wideBallRuns
          : 0;
        ...
        return {
          ...
          fallOfWickets: [
            ...(prev.fallOfWickets || []),
            {
              ...
              score: newRuns,
              wicket: newWickets,
              over: `${Math.floor(newBalls / settings.ballsPerOver)}.${
                newBalls % settings.ballsPerOver
              }`,
            },
          ],
        };
      ...
      setTeams((prevTeams) => {
        const updatedBatsmen = prevTeams[currentBattingTeamKey].batsmen.map(
          (b) => {
      ...
      // --- 7. Update UI ---
      const historyMark = isNoBall
        ? `${runsCompleted > 0 ? runsCompleted : ""}NB+W`
        : isWide
        ? `${runsCompleted > 0 ? runsCompleted : ""}WD+W`
        : "W";
...
const startSecondInnings = useCallback(() => {
  ...
  const firstInningsSummary = {
    ...
    bowling: formatBowlersForSummary(teams[firstInningsBowlingTeamKey].bowlers, settings.ballsPerOver),
    extras: match.extras,
    fallOfWickets: match.fallOfWickets,
  };

  const continueAction = () => {
    const secondInningsBattingTeamName = match.bowlingTeam;
    const secondInningsBowlingTeamName = match.battingTeam;

    const secondInningsBattingTeamKey =
      teams.teamA.name === secondInningsBattingTeamName ? 'teamA' : 'teamB';
    const secondInningsBowlingTeamKey =
      teams.teamA.name === secondInningsBowlingTeamName ? 'teamA' : 'teamB';

    // Reset only spell stats, not the bowler identity
    setTeams(prevTeams => {
      const newTeams = JSON.parse(JSON.stringify(prevTeams));
      newTeams[secondInningsBowlingTeamKey].bowlers = newTeams[secondInningsBowlingTeamKey].bowlers.map(bowler => ({
        ...bowler,
        previousSpells: [],
        currentSpell: { runs: 0, wickets: 0, balls: 0, maidens: 0 },
      }));
    ...
    setPlayers({
      striker: teams[secondInningsBattingTeamKey].batsmen[0],
      nonStriker: teams[secondInningsBattingTeamKey].batsmen[1],
      bowler: freshBowler,
      lastBowler: null,
    });
  ...
  setSummaryData({
    ...
    summaryLines: [
      { label: 'Team', value: firstInningsSummary.teamName },
      { label: 'Score', value: `${firstInningsSummary.runs} / ${firstInningsSummary.wickets}` },
      { label: 'Overs', value: firstInningsSummary.overs },
      { label: 'Target to Win', value: `${match.runs + 1}` },
    ],
    continueAction: continueAction
  });
}, [match, teams, players, settings, allOversHistory, overHistory]);
...
const endMatch = useCallback(async () => {
  const {
    ...
    overHistory: currentOverHistory,
    allOversHistory: currentAllOversHistory,
  ...
  if (!finalMatchId) {
    alert("A critical error occurred: Match ID is missing.");
    return;
  }
  ...
  const finalResult = calculateFinalResult(currentMatch, currentSettings);

  // --- 2. Innings 1 Data ---
  const innings1Data = currentMatch.firstInningsSummary;
  if (!innings1Data) {
    alert("Critical Error: First innings summary is missing.");
    return;
  }
  const completeHistoryForInnings1 = currentAllOversHistory.filter(o => o.innings === 1);

  // --- 3. Innings 2 Over History ---
  const completeHistoryForInnings2 = currentAllOversHistory.filter(o => o.innings === 2);
  if (currentOverHistory.length > 0) {
    completeHistoryForInnings2.unshift({
      overNumber: Math.floor(currentMatch.balls / currentSettings.ballsPerOver) + 1,
      bowlerName: currentPlayers.bowler.name,
      balls: [...currentOverHistory],
      innings: 2,
    });
  }

  // --- 4. Figure out 2nd innings batting & bowling team keys ---
  const secondInningsBattingTeamName = currentMatch.battingTeam;
  const secondInningsBowlingTeamName = currentMatch.bowlingTeam;

  const secondInningsBattingTeamKey =
    currentTeams.teamA.name === secondInningsBattingTeamName ? "teamA" : "teamB";
  const secondInningsBowlingTeamKey =
    currentTeams.teamA.name === secondInningsBowlingTeamName ? "teamA" : "teamB";

  // --- 5. Second Innings Data ---
  const innings2Data = {
    teamName: secondInningsBattingTeamName,
    battingTeam: secondInningsBattingTeamName,
    bowlingTeam: secondInningsBowlingTeamName,
    ...
    runRate:
      currentMatch.balls > 0
        ? (currentMatch.runs / (currentMatch.balls / currentSettings.ballsPerOver)).toFixed(2)
        : "0.00",
    batting: formatBatsmenForSummary(currentTeams[secondInningsBattingTeamKey].batsmen),
    bowling: formatBowlersForSummary(
      currentTeams[secondInningsBowlingTeamKey].bowlers,
      currentSettings.ballsPerOver
    ),
    extras: currentMatch.extras,
    fallOfWickets: currentMatch.fallOfWickets || [],
    target: currentMatch.target,
    overHistory: completeHistoryForInnings2,
  };

  // --- 6. Top Performers ---
  const topScorer = [...(innings1Data.batting || []), ...(innings2Data.batting || [])]
    .reduce((prev, curr) => (curr.runs > prev.runs ? curr : prev), { name: 'N/A', runs: -1 });

  const bestBowler = [...(innings1Data.bowling || []), ...(innings2Data.bowling || [])]
    .sort((a, b) => b.wickets - a.wickets || a.runs - b.runs)[0] || { name: 'N/A', wickets: 0, runs: 0 };
  ...
  const continueAction = async () => {
    const finalMatchDataPayload = {
      _id: finalMatchId,
      teamA: {
        name: currentTeams.teamA.name,
        shortName: currentTeams.teamA.shortName || currentTeams.teamA.name.substring(0, 3).toUpperCase()
      },
      teamB: {
        name: currentTeams.teamB.name,
        shortName: currentTeams.teamB.shortName || currentTeams.teamB.name.substring(0, 3).toUpperCase()
      },
      date: new Date().toISOString(),
      venue: matchSettings?.venue || "Unknown Venue",
      matchType: matchSettings?.matchType || "T20",
      toss: matchSettings?.toss || { winner: currentTeams.teamA.name, decision: "bat" },
      result: finalResult,
      status: "completed",
      innings1: {
        ...innings1Data,
        teamName: innings1Data.teamName || currentTeams.teamA.name,
        battingTeam: innings1Data.battingTeam || currentTeams.teamA.name,
        bowlingTeam: innings1Data.bowlingTeam || currentTeams.teamB.name,
        batting: innings1Data.batting || [],
        bowling: innings1Data.bowling || [],
        extras: innings1Data.extras || { total: 0 },
        fallOfWickets: innings1Data.fallOfWickets || [],
        overHistory: innings1Data.overHistory?.length > 0 ? innings1Data.overHistory : completeHistoryForInnings1
      },
      innings2: {
        ...innings2Data,
        teamName: innings2Data.teamName || currentTeams.teamB.name,
        batting: innings2Data.batting || [],
        extras: innings2Data.extras || { total: 0 },
    ...
    try {
      if (!finalMatchId.startsWith("guest_") && user?.token) {
        ...
        console.log("Final match payload:", finalMatchDataPayload);
        const response = await axios.put(
          `${API_BASE}/api/matches/${finalMatchId}/end`,
          finalMatchDataPayload,
          config
        );
      ...
      alert("An error occurred while saving the final match results. Please check the console for details.");
    }
  };
  ...
  setSummaryData({
    ...
    result: finalResult,
    summaryLines: [
      { label: 'Top Scorer', value: `${topScorer.name} (${topScorer.runs})` },
      { label: 'Best Bowler', value: `${bestBowler.name} (${bestBowler.wickets}/${bestBowler.runs})` },
    ],
    continueAction: continueAction,
  });

}, [matchSettings, onMatchEnd, user, stateRef]);
  ...
  const handleSummaryContinue = () => {
      if (summaryData && typeof summaryData.continueAction === 'function') {
          summaryData.continueAction();
      }
      setSummaryData(null); // Hide the modal
  };
    ...
    const loadMatch = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `${API_BASE}/api/matches/${currentMatchId}`,
          config
        );
  ...
  useEffect(() => {
    ...
    if (stateHasChanged()) {
      // Implement this function to check for meaningful changes
      debounceTimeout.current = setTimeout(() => {
        const saveUpdate = async () => {
          try {
            ...
            await axios.put(
              `${API_BASE}/api/matches/${match._id}`,
              {
                settings,
                match,
                teams,
  ...
  useEffect(() => {
    ...
    const isOverComplete =
      currentBallCount > 0 && currentBallCount % ballsPerOver === 0;

    if (
      isOverComplete &&
      currentBallCount !== lastCompletedOverBallCount.current
    ) {
      ...
      setTeams((prevTeams) => {
        const updatedTeams = JSON.parse(JSON.stringify(prevTeams));
        const bowlerToUpdate = updatedTeams[
          currentBowlingTeamKey
        ]?.bowlers.find((b) => b.id === bowlerId);
        if (bowlerToUpdate) {
          bowlerToUpdate.previousSpells = [
            ...(bowlerToUpdate.previousSpells || []),
            { ...bowlerToUpdate.currentSpell },
          ];
          bowlerToUpdate.currentSpell = {
      ...
      setPlayers((prevPlayers) => {
        const allBowlers = teams[currentBowlingTeamKey].bowlers;
        const currentBowlerIndex = allBowlers.findIndex(
          (b) => b.id === prevPlayers.bowler.id
        );
        let nextBowlerFromList =
          allBowlers[(currentBowlerIndex + 1) % allBowlers.length];
    ...
    players,
    saveState,
  ]); // Note: saveState is a dependency
  ...
  const getMatchResult = useCallback(() => {
    ...
    if (!match.isChasing && match.innings === 1)
  ...
  const determineWinner = useCallback(() => {
    ...
    if (match.target > match.runs)
      return match.firstInningsSummary?.teamName || "";
    return "Tie";
  }, [match]);
  ...
  const formatBowlersForSummary = (bowlersList = [], ballsPerOverSettings) => {
    return bowlersList.map((b) => {
      ...
      return {
        name: b.name,
        overs: `${Math.floor(totalBalls / ballsPerOverSettings)}.${
          totalBalls % ballsPerOverSettings
        }`,
        ...
        economyRate:
          totalBalls > 0
            ? ((totalRunsConceded / totalBalls) * ballsPerOverSettings).toFixed(
                2
              )
            : "0.00",
      };
  ...
  const formatBowlersForUpdate = (bowlersList = [], ballsPerOverSettings) => {
    return bowlersList.map((b) => {
      ...
      const allSpells = [...(b.previousSpells || [])];
      if (b.currentSpell) {
        // Always include currentSpell, even if balls are 0 for a new bowler
        allSpells.push(b.currentSpell);
      }
  ...
  const getNextBatsman = (battingTeamKey, nonStrikerId, teamsData) => {
    return teamsData[battingTeamKey].batsmen.find(
      (b) => !b.isOut && b.id !== nonStrikerId
    );
  };
  ...
  const handleConfirmExit = () => {
    if (onExitMatch) {
      // Assuming onExitMatch is a prop
      onExitMatch();
    }
  ...
  const handleRetire = (type, batter) => {
    ...
    const outBatsmanId =
      batter === "Striker" ? players.striker.id : players.nonStriker.id;
  ...
  const handleBowlerChange = (newBowlerId) => {
    ...
    setTeams((prevTeams) => {
      const newTeams = JSON.parse(JSON.stringify(prevTeams));
      const bowlerToUpdate = newTeams[currentBowlingTeamKey]?.bowlers.find(
        (b) => b.id === bowlerToChangeFromId
      );
    ...
    const newBowlerFromList = teams[currentBowlingTeamKey].bowlers.find(
      (b) => b.id === newBowlerId
    );
  ...
  const handleOK = () => {
    ...
    if (showModal === "OUT") {
      ...
      if (dismissalInfo.dismissalType === "Run Out") {
        if (
          dismissalInfo.runsCompleted === undefined ||
          !dismissalInfo.batsmanOut
        ) {
          alert(
            "For a Run Out, please select both the runs completed and which batsman was out."
          );
          return;
        }
        handleWicket({
  ...
  const formatBatsmenData = (
    ...
    return batsmenListFromTeamsState.map((batsmanInTeam) => {
      ...
      let atCreaseStatus = false;
      if (!isEffectivelyOut) {
        // Only not-out players can be at crease
        if (
          currentStrikerFromPlayersState &&
          batsmanInTeam.id === currentStrikerFromPlayersState.id
        ) {
  ...
  const startEditingPlayer = (playerType, playerId, team) => {
    ...
    const playerList =
      playerType === "batsman" ? teamData.batsmen : teamData.bowlers;
  ...
  const savePlayerName = () => {
    ...
    const authoritativePlayer = teams[team][playerArrayKey].find(
      (p) => p.id === id
    );
  ...
  const BowlerInfo = ({ bowler }) => {
    ...
    const currentSpell = bowler.currentSpell || {
      runs: 0,
      wickets: 0,
      balls: 0,
      maidens: 0,
    ...
    const totalBalls =
      previousSpells.reduce((sum, spell) => sum + spell.balls, 0) +
      currentSpell.balls;
    const totalOvers = `${Math.floor(totalBalls / settings.ballsPerOver)}.${
      totalBalls % settings.ballsPerOver
    }`;
    const totalRuns =
      previousSpells.reduce((sum, spell) => sum + spell.runs, 0) +
      currentSpell.runs;
    const totalWickets =
      previousSpells.reduce((sum, spell) => sum + spell.wickets, 0) +
      currentSpell.wickets;
    const economyRate =
  ...
  const BowlerDisplay = ({ bowler, ballsPerOver }) => {
    ...
    const allSpells = [
      ...(bowler.previousSpells || []),
      ...(bowler.currentSpell ? [bowler.currentSpell] : []),
    ];

    const totalStats = allSpells.reduce(
      (acc, spell) => ({
        runs: acc.runs + (spell.runs || 0),
        wickets: acc.wickets + (spell.wickets || 0),
        balls: acc.balls + (spell.balls || 0),
    ...
    const overs = `${Math.floor(totalStats.balls / ballsPerOver)}.${
      totalStats.balls % ballsPerOver
    }`;
    const economy =
      totalStats.balls > 0
      ...
      <div className="bowler-display">
        <div className="bowler-header">
          ...
          >
            ...
            {players.bowler ? players.bowler.name : "N/A"}{" "}
            <span className="text-xs">✏️</span>
          </p>
          <div className="bowler-stats">
            <span>
              {totalStats.runs}/{totalStats.wickets}
            </span>
            <span>({overs})</span>
          </div>
          <button
  ...
  if (showPrevOversModal) {
    const currentInningsHistory = allOversHistory.filter(
      (over) => over.innings === match.innings
    );
    ...
    return (
      <div className="modal open">
        ...
        <motion.div
          className="modal-content-wrapper previous-overs-modal" // Use a wrapper for animation
        ...
        >
          <div className="modal-content" role="dialog" aria-modal="true">
            <header className="modal-header">
              <h3>
                {match.innings === 1 ? "1st" : "2nd"} Innings History
              </h3>
            ...
            <div className="previous-overs-list">
              {currentInningsHistory.length === 0 ? (
                <p className="no-history-message">
                  No completed overs for this innings yet.
                </p>
              ) : (
                currentInningsHistory.map((over) => (
                  <motion.div
                    key={over.overNumber}
                    className="over-summary-item"
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: over.overNumber * 0.05 }}
                  >
                    <div className="over-summary-header">
                      <span className="over-number">Over {over.overNumber}</span>
                      <span className="bowler-name">{over.bowlerName}</span>
                      <span className="over-runs-wickets">
                    ...
                    <div className="balls-container-modal">
                      {over.balls.map((ball, index) => (
                        <span
                          key={index}
                          className={`ball-mark ${
                            ball.toString().includes("W") ? "wicket"
                            : ball === "4" || ball === "6" ? "boundary"
                            : ball.includes("WD") || ball.includes("NB") || ball.includes("BYE") || ball.includes("LB") ? "extra"
                            : ""
                          }`}
                        >
                          {ball}
    ...
    if (showModal === "EDIT_PLAYER") {
      return (
        <div className="modal open">
          ...
          <div className="modal-content" role="dialog" aria-modal="true">
            <h3>
              Edit {editingPlayer?.type === "batsman" ? "Batsman" : "Bowler"}{" "}
              Name
            </h3>
            <input
              type="text"
    ...
    if (showModal === "CHANGE_BOWLER") {
      ...
      const availableBowlers = teams[currentBowlingTeam].bowlers
        ...
        .map((bowler) => ({
          ...bowler,
          overs: `${Math.floor(bowler.balls / settings.ballsPerOver)}.${
            bowler.balls % settings.ballsPerOver
          }`,
        }));
      ...
      return (
        <div className="modal open">
          ...
          <div className="modal-content" role="dialog" aria-modal="true">
              ...
              {availableBowlers.map((bowler) => (
                <div
                  ...
                  className={`bowler-option ${
                    players.bowler.id === bowler.id ? "current-bowler" : ""
                  }`}
                  onClick={() => {
            ...
            <div className="modal-actions">
              <button onClick={() => setShowModal(null)} className="cancel-btn">
                Cancel
              </button>
            </div>
          </div>
    ...
    const renderRunOutDetails = () => (
      <div className="run-out-details">
        ...
        <div className="number-grid">
          {[0, 1, 2, 3, 4, 5].map((num) => (
            <button
              key={num}
              onClick={() =>
                setModalData((prev) => ({ ...prev, runsCompleted: num }))
              }
        ...
        <div className="batter-selection">
          <button
            onClick={() =>
              setModalData((prev) => ({ ...prev, batsmanOut: "striker" }))
            }
            className={`option-button ${
              modalData.batsmanOut === "striker" ? "selected" : ""
            }`}
          >
          ...
          <button
            onClick={() =>
              setModalData((prev) => ({ ...prev, batsmanOut: "nonStriker" }))
            }
            className={`option-button ${
              modalData.batsmanOut === "nonStriker" ? "selected" : ""
            }`}
          >
    ...
    const renderOutOptions = () => (
      <div className="out-options">
        {["Bowled", "Caught", "Run Out", "LBW", "Stumped", "Hit Wicket"].map(
          (type) => (
            <button
              key={type}
              onClick={() =>
                setModalData((prev) => ({ ...prev, dismissalType: type }))
              }
        ...
        <div className="direct-wicket-section">
          ...
          <div className="button-group">
            <button
              ...
              className={`option-button ${
                modalData.isRunOutFlow ? "selected" : ""
              }`}
            >
              Run Out
            </button>
        ...
        <div className="batter-selection">
          ...
          <div className="button-group">
            {[
              ...
              {
                label: "Non-Striker",
                name: players.nonStriker?.name || "Non-Striker",
              },
            ].map(({ label, name }) => (
              <button
                key={label}
                onClick={() =>
                  setModalData((prev) => ({ ...prev, batter: label }))
                }
                className={`option-button ${
                  modalData.batter === label ? "selected" : ""
                }`}
      ...
      if (editingTeam && editingTeam !== "choose") {
        return (
          <div className="edit-players-screen">
            <h3>
              Edit Player Names -{" "}
              {editingTeam === "teamA" ? "Team A" : "Team B"}
            </h3>
            {tempNames.map((player, index) => (
              <div key={player.id} className="setting-item">
                ...
                <input
                  ...
                  value={player.name}
                  onChange={(e) =>
                    handleTempNameChange(player.id, e.target.value)
                  }
                />
              </div>
            ))}
        ...
        <div className="settings-options">
          ...
          <div className="setting-item">
            ...
            <input
              ...
              onChange={(e) =>
                handleChangeExtraRuns("WD", parseInt(e.target.value) || 1)
              }
            />
          </div>
          ...
          <div className="setting-item">
            ...
            <input
              ...
              onChange={(e) =>
                handleChangeExtraRuns("NB", parseInt(e.target.value) || 1)
              }
            />
          </div>
          ...
          <div className="setting-item">
            ...
            <input
              ...
              onChange={(e) =>
                handleChangeBallsPerOver(parseInt(e.target.value) || 6)
              }
            />
          </div>
  ...
  return (
    <div className="live-score-container">
      ...
      <ScorecardHeader onBack={handleBackButtonConfirm} />
          ...
          >
            <div className="confirm-modal-icon">
              ...
              >
                <path
                  fillRule="evenodd"
                  d="M9.401 3.003c1.155-2 4.043-2 5.197 0l7.355 12.748c1.154 2-.772 4.5-3.298 4.5H5.344c-2.526 0-4.45-2.5-3.298-4.5L9.4 3.003zM12 8.25a.75.75 0 01.75.75v3.75a.75.75 0 01-1.5 0V9a.75.75 0 01.75-.75zm0 8.25a.75.75 0 100-1.5.75.75 0 000 1.5z"
                  clipRule="evenodd"
                />
              </svg>
            </div>
      ...
      <div className="main-content-section">
        <div className="scorecard-section">
          ...
          <InningsInfo
              ...
              onTeamNameChange={handleTeamNameUpdate}
          />
          ...
          {match.isChasing && !match.isComplete && (
            <div className="target-info">
              <div className="target-row highlight">
                <span>
                  Need {Math.max(0, remainingRuns)} runs from {remainingBalls}{" "}
                  balls
                </span>
              </div>
          ...
          <BatsmanInfo
              ...
              onNameChange={handleBatsmanNameUpdate}
          />
          ...
          <div className="bowler-info">
            <BowlerDisplay
              bowler={teams[
                match.innings === 1 ? "teamB" : "teamA"
              ].bowlers.find((b) => b.id === players.bowler.id)}
        ...
        {summaryData && (
            <SummaryModal
                ...
                summaryLines={summaryData.summaryLines}
            ...
            <div className="button-row">
              {["WD", "NB", "BYE", "UNDO"].map((btn) => (
                <button
                  key={btn}
                  onClick={() =>
                    btn === "UNDO" ? handleUndo() : handleActionClick(btn)
                  }
